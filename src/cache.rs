...
impl RingCache {
    ...
    pub fn get(&self, key: &str) -> Option<Bytes> {
        ...
        match self.file.read_exact_at(&mut buf, off) {
            ...
            // A short read means the region was reused. Treat as a miss.
            Err(_) => {
                self.misses.fetch_add(1, Ordering::Relaxed);
                None
            }
    ...
    pub fn put(&self, key: &str, data: &[u8]) {
        ...
        while let Some(front) = ring.order.front().cloned() {
            let Some(&(off, l)) = ring.map.get(&front) else {
        ...
        ring.map.insert(key.to_string(), (start, data.len() as u32));
